...
import 'package:motel/presentation/settings/service_edit_screen.dart';
...
class _ServicesSettingsScreenState extends State<ServicesSettingsScreen> {
          ...
          CustomScrollView(
            slivers: [
              const CupertinoSliverNavigationBar(
                ...
                // --- ИЗМЕНЕНИЕ ЗДЕСЬ: trailing убран ---
              ),
              CupertinoSliverRefreshControl(onRefresh: _loadServices),

              // --- ИЗМЕНЕНИЕ ЗДЕСЬ: Вся логика отображения перенесена сюда ---
              _buildServiceList(),

              // --- ИЗМЕНЕНИЕ ЗДЕСЬ: Добавлена новая секция с кнопкой ---
              SliverToBoxAdapter(
  ...
  // Вспомогательный метод, чтобы не загромождать `build`
